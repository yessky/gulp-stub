;!function(kjs) {
	...
	function mixOwn(target, source) {
		...
		if ( has('bug-for-in-skips-shadowed') ) {
			for ( var shadowNames = mix._shadowNames, i = shadowNames.length; i; ) {
				name = shadowNames[--i];
				if ( name !== cname && source.hasOwnProperty(name) ) {
					  target[name] = source[name];
				}
	...
	function declare( className, superclass, props ) {
		...
		if ( className ) {
			...
			if (!declare.declaredClass) {
				declare.declaredClass = {};
			}
			declare.declaredClass[className] = ctor;
		}
	...
	kjs.getInstanceOf = function(className, params, referNode) {
		...
		return new Class(params, referNode);
	};
